	...
	struct cam_eeprom_memory_block_t *block)
{
	...
	for (j = 0; j < block->num_map; j++) {
        ...
        //hisense add
        if (emap[j].memr.valid_size) {
            for (i=0; i<emap[j].memr.valid_size; i++) {
                rc = camera_io_dev_read_seq(&e_ctrl->io_master_info,
                    emap[j].memr.addr, memptr,
                    emap[j].memr.addr_type,
                    emap[j].memr.data_type,
                    1);
                if (rc < 0) {
                    CAM_ERR(CAM_EEPROM, "memr read failed rc %d",rc);
                    return rc;
                }
                memptr += 1;
            }
	...
	int *num_map, size_t remain_buf_len)
{
	...
	else if (cmm_hdr->cmd_type == CAMERA_SENSOR_CMD_TYPE_I2C_CONT_RD_SINGLE) //hisense add
		validate_size = sizeof(struct cam_cmd_i2c_continuous_rd);
	...
	case CAMERA_SENSOR_CMD_TYPE_I2C_CONT_RD:
		...
		break;
	case CAMERA_SENSOR_CMD_TYPE_I2C_CONT_RD_SINGLE: //hisense add
		i2c_cont_rd = (struct cam_cmd_i2c_continuous_rd *)cmd_buf;
		cmd_length_in_bytes = sizeof(struct cam_cmd_i2c_continuous_rd);

		if (i2c_cont_rd->header.count >= U32_MAX - data->num_data) {
			CAM_ERR(CAM_EEPROM,
				"int overflow on eeprom memory continuous read  block");
			return -EINVAL;
		}
		map[*num_map].memr.addr = i2c_cont_rd->reg_addr;
		map[*num_map].memr.addr_type = i2c_cont_rd->header.addr_type;
		map[*num_map].memr.data_type = i2c_cont_rd->header.data_type;
		map[*num_map].memr.valid_size =
			i2c_cont_rd->header.count;
		cmd_buf += cmd_length_in_bytes / sizeof(int32_t);
		processed_size +=
			cmd_length_in_bytes;
		data->num_data += map[*num_map].memr.valid_size;
		break;
	case CAMERA_SENSOR_CMD_TYPE_WAIT:
		if (generic_op_code ==
			...
			map[*num_map - 1].memr.delay = i2c_uncond_wait->delay;//hisense add
